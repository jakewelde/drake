...
blur_halfwidth = size(background_2D,1)/40;
...
filter_2D = trainConvLS(kernel_2D, depth_imgs_2D, ref_imgs_2D, num_samps);
...
%     recon_2D = recon_2D + background_2D_recon;
    ...
    for color1=1:size(depth_img_2D,3)
        for color2=1:size(ref_img_2D,3)
            recon_2D(:,:,color2) = recon_2D(:,:,color2) + conv2(depth_img_2D(:,:,color1), filter_2D(:,:,color1,color2), 'same');
        end
    end
    recon_2D = max(0, min(1, recon_2D));
    
    next_fig = cat(1, repmat(depth_img_2D,[1 1 3]), ref_img_2D, recon_2D);
    if isempty(full_fig)
        full_fig = next_fig;
    else
