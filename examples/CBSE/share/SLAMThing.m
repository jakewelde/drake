classdef SLAMThing < MIMODrakeSystem
  ...
  methods
    ...
    function [utility, j] = objective_fun(obj, x)
      % This is no longer in use, it was present for a prototype
      % formulation of this as a straight-up nonlinear solve through
      % Snopt.
    ...
    function [integration_constraint, integration_equality, manip_constraint, manip_equality, imu_constraint, imu_equality] = ISAM_generate_constraints(obj, n, nX, m)
      ...
      if (n~=1)
        ...
        [phi,normal,~,~,~,~,~,~,n,D,dn,dD] = obj.r.contactConstraints(state_hist(1:6, end),false, obj.active_collision_options);
        ...
        manip_constraint(:, nX_per+6+offset:nX_per+12+offset) = Hnext;
      ...
      %out = out*0;
      if (n~=1)
        imu_constraint(:, offset-m-6:offset-m) = -out;
      end
    ...
    function doISAM(obj)
      ...
      if size(A, 1) > 0
        ...
        if (isempty(d) || isempty(R) || mod(N, 25) == 0)
          ...
          Areg = [zeros(12+obj.nContactForces, size(A, 2)); A];
          Areg(1:12+obj.nContactForces, 1:12+obj.nContactForces) = eye(12+obj.nContactForces);
% x0.base_z = 10;
% x0.base_zdot = 0;
% x0.base_pitch = 0.5;
% x0.base_roll = -0.5;
